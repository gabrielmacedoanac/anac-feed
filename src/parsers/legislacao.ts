...
export async function fetchLegislacao(): Promise<ContentItem[]> {
  try {
    ...
    // 🚀 NOVO: Usa Deno.run com cURL e o parâmetro --resolve
    // O --resolve força o DNS a usar o IP que você especificar, contornando bloqueios de IP de datacenter.
    // O cURL lida nativamente com o certificado HTTPS quando se usa o --resolve.
    const process = Deno.run({
        cmd: [
            "curl",
            "-s", // Silencioso
            "-k", // Permite conexões inseguras (apenas para debug, pode ser removido depois)
            "-H", `User-Agent: ${USER_AGENT}`,
            // O parâmetro --resolve força o domínio a resolver para um IP específico
